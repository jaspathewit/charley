...
POPULATION *mate(POPULATION *temp, NETWORK_PARAM *params) {
	...
	new_pop = create_population(params->population_size);
	if (new_pop != NULL) {
...
Function selects a crossover point on the two net works and exchanges links between them
...
static void crossover(NETWORK *network1, NETWORK *network2, NETWORK_PARAM *params) {
	...
	/* get the crossover node for network 1 */
	...
	/* get the crossover point for network 2*/
...
Function rounds a floating point number up or down  to an appropriate integer

Function takes the floating point number to round

Function returns the appropriate integer
...
*/
static unsigned int cround(float number) {
	float result = 0;
...
static NETWORK *adddel_mutation(NETWORK *network, NETWORK_PARAM *params) {
	...
	/* decide whether it's an addition or deletion  */
