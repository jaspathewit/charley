/* File contains functions that create and destroy route link structures
...
void destroy_route_link(ROUTE_LINK *);
...
ROUTE_LINK *create_route_link(void) {
	...
	temp = (ROUTE_LINK *) malloc(sizeof(ROUTE_LINK));
	if (temp != NULL) {
...
/* Function destroy_route_link

Function frees the memory allocated to a route_link structure

Function takes a pointer to the route_link structure to destroy
...
*/
void destroy_route_link(ROUTE_LINK *this_route_link) {
	/* logger freed memory */
#ifdef DEBUG_MEM
