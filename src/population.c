/* File contains functions that create and destroy population structures
...
void destroy_population(POPULATION *, unsigned);
...
POPULATION *create_population(unsigned no_networks) {
    ...
    logger("Population allocation %p",temp);
		...
		networks = (NETWORK **) calloc(no_networks, sizeof(NETWORK *));
		if (networks != NULL) {
			/* logger allocation successful */
#ifdef DEBUG_MEM
...
Function takes a pointer to the population to destroy and
...
*/
void destroy_population(POPULATION *this_pop, unsigned no_networks) {
	unsigned count = 0;

	/* check there is a population to destroy */
	if (this_pop) {
		/* destroy the networks	stored in the population */
		for (count = 0; count < no_networks; count++) {
			/* destroying network */
			destroy_network(this_pop->networks[count]);
		}
