...
static char *get_sub_expression(char *expression, int no_tabs) {
	...
	if (temp_expression) {
		...
		sub_expression = (char *) calloc(strlen(temp_expression) + no_tabs + 1, sizeof(char));
		...
		if (sub_expression) {
			...
			*sub_expression = EMPTYC;
			...
			for (int count = 1; count <= no_tabs; count++) {
