/* File contains functions that create and destroy connectivity records
...
void destroy_con_rec(CONNECTIVITY_REC *);
...
CONNECTIVITY_REC *create_con_rec(void) {
	...
	temp = (CONNECTIVITY_REC *) malloc(sizeof(CONNECTIVITY_REC));
	if (temp != NULL) {
...
/* Function destroy_con_rec

Function frees the memory allocated to a connectivity rec

Function takes a pointer to the structure to destroy
...
*/
void destroy_con_rec(CONNECTIVITY_REC *this_node) {
	/* logger freed memory */
#ifdef DEBUG_MEM
