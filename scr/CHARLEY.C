...
void main(int no_args, char *args[])   
{      
...
if(check_args(no_args,args))   
  {   
    ...
    /* initilize the network parameters from the ini file */   
    if(network_params=charley_init(args[1]))   
      {   
	...
	if(temp)  
	  {  
	    ...
	    /* evolve the population network */   
		 ...
		 if(!(temp=evolve(temp,network_params))) 
	      {   
	    ...
	    distroy_population(temp,network_params->population_size);   
	  }  
        /* delete the network parameters */   
        distroy_network_param(network_params);   
      }    
  }  
logger("***********Program end***********");   
