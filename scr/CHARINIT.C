...
extern void distroy_network_param(NETWORK_PARAM *);  
...
if(ini_file=fopen(file_name,"r"))  
	{	  
	/* fill up the structure */                                  
	logger("Initilisation started");  
	/*printf("\nStarted initilisation"); */ 
		  
	if(temp=fill_up(ini_file))  
		{                  
		logger("Initilisation finished");  
		/*printf("\nCompleted initilisation"); */ 
        }  
	...
	logger("Initilsation failed. Could not open file %s",file_name);  
...
Function fills the network_parameter structure with the contents of  
...
if(!success)  
	...
	/* distroy the network_parameters created so far*/  
	distroy_network_param(temp);  
	temp=NULL;  
	}   
...
/* copy the link */  
if(dup_link=copy_link(this_link))  
	{  
	/* record the success */  
	success=TRUE;		                    
