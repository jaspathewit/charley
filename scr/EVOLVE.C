...
for(generation=1;generation<=params->no_generations;generation++)   
  {   
    ...
    if(compare_networks(&temp_best_network, &best_network)==1)   
      {   
	/* distroy the current best network */   
	distroy_network(best_network); 
	...
	print_network(best_network);  
      ...
      {
	/* distroy the temp best network */
	distroy_network(temp_best_network);
      }
   
		      
    /* make the new population the current population */   
    distroy_population(current_pop, params->population_size);      
    current_pop=new_pop;   
    ...
    printf("\n%f %f %f",new_pop->mean_fitness, new_pop->best_fitness, new_pop->worst_fitness);
  }  
  
print_network(best_network);  
  
/* distroy the copy of the current best population */     
distroy_network(best_network);   
